...
namespace glr
{
  ...
  void Framebuffer::bind(Attachment type, uint32_t target) const
  {
    ...
    {
      case Attachment::COLOR:
        glBindTextureUnit(target, this->m_colorHandle);
        break;
      case Attachment::DEPTH:
        glBindTextureUnit(target, this->m_depthHandle);
        break;
      case Attachment::STENCIL:
        glBindTextureUnit(target, this->m_stencilHandle);
        break;
      default:
        break;
    }
